...
def is_sorted(sr):
    ...
    while True:
        s = sr.next()
        if s == io.EOF: return True
        if score and score > s.score: return i
        score = s.score
        i += 1
...
def test_sort(sorting, fname=None, n=16384):
        ...
        if not fname: gen(f, n, close=True)
        t = time.time()
        sorting(f)
        t = time.time() - t
        reader = sr(f)
        ...
        reader.close()
        if r == True:
            print('[+] successfully sorted random data with N=%d in %f sec.'%(n, t))
        else:
            print('[-] sorting on random input failed, disorder: line %d'%r)
        io.rm(f)
        return r == True # == True is necessary since r being integer means error
...
def main_check(args):
    ...
    r  = is_sorted(sr)
    if r == True: print('[+] file is sorted')
    else:
...
def main():
    ...
    parser = argparse.ArgumentParser(\
            description='Task10 (students rating/external sorting) testing suite')
    ...
    parser_gen.add_argument('-o', dest='out', type=argparse.FileType('w'), default='task10.rnd.in')
    parser_gen.add_argument('-n', dest='n', type=int, default=4096, help='number of students to generate')
    parser_gen.set_defaults(cmd=main_gen)

    parser_check = subparsers.add_parser('check', help='check if file is sorted')
